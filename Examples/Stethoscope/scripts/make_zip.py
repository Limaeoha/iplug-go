...
def  main():
  ...
  installer = "\installer\Stethoscope Installer.exe"
   
  if demo:
    installer = "\installer\Stethoscope Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Stethoscope manual.pdf" 
  ]
  ...
  ZIPNAME = "Stethoscope-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Stethoscope-v" + FULLVERSIONSTR + "-win-demo.zip"

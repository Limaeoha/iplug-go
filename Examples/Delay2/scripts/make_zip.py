...
def  main():
  ...
  installer = "\installer\Delay2 Installer.exe"
   
  if demo:
    installer = "\installer\Delay2 Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Delay2 manual.pdf" 
  ]
  ...
  ZIPNAME = "Delay2-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Delay2-v" + FULLVERSIONSTR + "-win-demo.zip"

...
def  main():
  ...
  installer = "\installer\BPM Installer.exe"
   
  if demo:
    installer = "\installer\BPM Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\BPM manual.pdf" 
  ]
  ...
  ZIPNAME = "BPM-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "BPM-v" + FULLVERSIONSTR + "-win-demo.zip"
